...
The BNX file was produced from the CHM13 cell line and does not include the Y chromosome. Due to certain sites in the genome not containing restrictions sites, the BNX data has gaps and is not telomere to telomere. The cmap was generated from the BNX file using Bionano's software package known as bionano-solve. The cmap file therefore also has gaps and does not include Y chromosome data. However, if needed, bionano has a tool that can convert the complete T2T sequence into a format comatible with data output from the sapphyr instrument.
